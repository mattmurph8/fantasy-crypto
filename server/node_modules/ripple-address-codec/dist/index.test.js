...
[false, true].forEach(isTestAddress => {
    ...
    for (const i in testCases) {
        ...
        test(`Converts ${classicAddress}${tag ? ':' + tag : ''} to ${xAddress}${network}`, () => {
            expect(index_1.classicAddressToXAddress(classicAddress, tag, isTestAddress)).toBe(xAddress);
            const myClassicAddress = index_1.xAddressToClassicAddress(xAddress);
            expect(myClassicAddress).toEqual({
                classicAddress,
                tag,
                test: isTestAddress
            });
            expect(index_1.isValidXAddress(xAddress)).toBe(true);
        });
    }
    {
        ...
        test(`Converting ${classicAddress}:${tag}${network} throws`, () => {
            expect(() => {
                index_1.classicAddressToXAddress(classicAddress, tag, isTestAddress);
    ...
    }
    {
        ...
        test(`Invalid classic address: Converting ${classicAddress}${network} throws`, () => {
            expect(() => {
                index_1.classicAddressToXAddress(classicAddress, false, isTestAddress);
    ...
    }
    {
        ...
        highAndLowAccounts.forEach(accountId => {
            [false, 0, 1, MAX_32_BIT_UNSIGNED_INT].forEach(t => {
                const tag = t;
                const xAddress = index_1.encodeXAddress(accountId, tag, isTestAddress);
...
});
{
    ...
    test(`Invalid X-address (bad checksum): Converting ${xAddress} throws`, () => {
        expect(() => {
            index_1.xAddressToClassicAddress(xAddress);
...
test(`Invalid X-address (64-bit tag) throws`, () => {
    expect(() => {
        ...
        // }
        index_1.xAddressToClassicAddress('XVLhHMPHU98es4dbozjVtdWzVrDjtV18pX8zeUygYrCgrPh');
    }).toThrowError('Unsupported X-address');
});
test(`Invalid Account ID throws`, () => {
    expect(() => {
        index_1.encodeXAddress(Buffer.from('00'.repeat(19), 'hex'), false, false);
    }).toThrowError('Account ID must be 20 bytes');
});
test(`isValidXAddress returns false for invalid X-address`, () => {
    expect(index_1.isValidXAddress('XVLhHMPHU98es4dbozjVtdWzVrDjtV18pX8zeUygYrCgrPh')).toBe(false);
});
